...
class JSON  {
	...
	const encode = 'Basic\\JSON::encode';
	const output = 'Basic\\JSON::output';
	const sendHeader = 'Basic\\JSON::sendHeader';
	 ...
	 *
	 */
	private static $control_character_map = array ('"' => '\"', '\\' => '\\\\', '/' => '\/', "\x8" => '\b', "\xC" => '\f', "\n" => '\n', "\r" => '\r', "\t" => '\t' );
	 ...
	 *
	 * @param $json string
	 *       	 This should be the name of a related class
	 * @param $assoc boolean
	 *       	 If this is TRUE, JSON objects will be represented as an
	 *        	assocative array instead of a `stdClass` object
...
~x', $json, $matches );
		...
		foreach ( $matches [0] as $match ) {
			...
			if ($type == self::J_COLON || $type == self::J_OBJ_COMMA || $type == self::J_ARRAY_COMMA || $type == self::J_KEY) {
				$last = $type;
				if ($type == self::J_KEY) {
					$last_key = $match;
				}
			...
			// This flag is used to indicate if an array or object is being
			// added and thus
			// if the container reference needs to be changed to the current
			// match
			$ref_match = FALSE;
	...
	static public function encode($value) {
		...
		if (function_exists ( 'json_encode' )) {
			...
			if ($is_scalar || ! $has_json_serializable || version_compare (PHP_VERSION, '5.4') > 0) {
				//
				// Versions of PHP >= 5.4 have a native JSONSerializable
				// interface so we can
				// natively encode scalars, systems without that interface at
				// all, or those which
				// would have it natively.
				//
		...
		// Detect if an array is associative, which would mean it needs to be
	 ...
	 *
	 * @param
	 *       	 array &$stack The stack of arrays/objects being parsed
	 * @param $last integer
	 *       	 The type of the last element parsed
	 * @param $element string
	 *       	 The element being detected
	 ...
	 *
	 * @param $match array
	 *       	 A regex match containing the 4 digit code referenced by the
	 *        	key `1`
	...
	/**
	 * Sets the proper `Content-Type` header and outputs the value, encoded as
	 * JSON
	 *
	 * @param $value mixed
	 *       	 The PHP value to output as JSON
	 * @return void
	 *
	 */
	 ...
	 * @param $element string
	 *       	 The element to be converted to a scalar
