...
class Grammar  {
	 ...
	 *
	 */
	private static $cache = array ('camelize' => array (0 => array (), 1 => array () ), 'humanize' => array (), 'pluralize' => array (), 'singularize' => array (), 'underscorize' => array () );
	 ...
	 *
	 */
	private static $camelize_rules = array ();
	 ...
	 *
	 */
	private static $humanize_rules = array ();
	 ...
	 *
	 */
	private static $join_array_callback = NULL;
	 ...
	 *
	 */
	private static $plural_to_singular_rules = array ('([ml])ice$' => '\1ouse', '(media|info(rmation)?|news)$' => '\1', '(q)uizzes$' => '\1uiz', '(c)hildren$' => '\1hild', '(p)eople$' => '\1erson', '(m)en$' => '\1an', '((?!sh).)oes$' => '\1o', '((?<!o)[ieu]s|[ieuo]x)es$' => '\1', '([cs]h)es$' => '\1', '(ss)es$' => '\1', '([aeo]l)ves$' => '\1f', '([^d]ea)ves$' => '\1f', '(ar)ves$' => '\1f', '([nlw]i)ves$' => '\1fe', '([aeiou]y)s$' => '\1', '([^aeiou])ies$' => '\1y', '(la)ses$' => '\1s', '(.)s$' => '\1' );
	 ...
	 *
	 */
	private static $singular_to_plural_rules = array ('([ml])ouse$' => '\1ice', '(media|info(rmation)?|news)$' => '\1', '(phot|log|vide)o$' => '\1os', '^(q)uiz$' => '\1uizzes', '(c)hild$' => '\1hildren', '(p)erson$' => '\1eople', '(m)an$' => '\1en', '([ieu]s|[ieuo]x)$' => '\1es', '([cs]h)$' => '\1es', '(ss)$' => '\1es', '([aeo]l)f$' => '\1ves', '([^d]ea)f$' => '\1ves', '(ar)f$' => '\1ves', '([nlw]i)fe$' => '\1ves', '([aeiou]y)$' => '\1s', '([^aeiou])y$' => '\1ies', '([^o])o$' => '\1oes', 's$' => 'ses', '(.)$' => '\1s' );
	 ...
	 *
	 */
	private static $underscorize_rules = array ();
	
	/**
	 * Adds a custom mapping of a non-humanized string to a humanized string for
	 * ::humanize()
	 *
	 * @param $non_humanized_string string
	 *       	 The non-humanized string
	 * @param $humanized_string string
	...
	/**
	 * Adds a custom `camelCase` to `underscore_notation` and
	 * `underscore_notation` to `camelCase` rule
	 *
	 * @param $camel_case string
	 *       	 The lower `camelCase` version of the string
	 * @param $underscore_notation string
	 *       	 The `underscore_notation` version of the string
	 * @return void
	 *
	 */
	...
	/**
	 * Adds a custom singular to plural and plural to singular rule for
	 * ::pluralize() and ::singularize()
	 *
	 * @param $singular string
	 *       	 The singular version of the noun
	 * @param $plural string
	...
	static public function addSingularPluralRule($singular, $plural) {
		self::$singular_to_plural_rules = array_merge ( array ('^(' . preg_quote ( $singular [0], '#' ) . ')' . preg_quote ( substr ( $singular, 1 ), '#' ) . '$' => '\1' . strtr ( substr ( $plural, 1 ), array ('\\' => '\\\\', '$' => '\\$' ) ) ), self::$singular_to_plural_rules );
		self::$plural_to_singular_rules = array_merge ( array ('^(' . preg_quote ( $plural [0], '#' ) . ')' . preg_quote ( substr ( $plural, 1 ), '#' ) . '$' => '\1' . strtr ( substr ( $singular, 1 ), array ('\\' => '\\\\', '$' => '\\$' ) ) ), self::$plural_to_singular_rules );
	...
	/**
	 * Converts an `underscore_notation`, human-friendly or `camelCase` string
	 * to `camelCase`
	 *
	 * @param $string string
	 *       	 The string to convert
	 * @param $upper boolean
	 *       	 If the camel case should be `UpperCamelCase`
	 * @return string The converted string
	 *        
	 */
	static public function camelize($string, $upper) {
		if (! strlen ( $string )) {
			throw new \Exception ( "An empty string was passed to %s", __CLASS__ . '::camelize()' );
		}
	...
	/**
	 * A callback used by ::camelize() to handle converting underscore to
	 * camelCase
	 *
	 * @param $match array
	...
	/**
	 * Makes an `underscore_notation`, `camelCase`, or human-friendly string
	...
	static public function humanize($string) {
		if (! strlen ( $string )) {
			throw new \Exception ( "An empty string was passed to %s", __CLASS__ . '::humanize()' );
		}
		...
		if (isset ( self::$humanize_rules [$string] )) {
			...
			$string = preg_replace_callback ( '/(\b(api|css|gif|html|id|jpg|js|mp3|pdf|php|png|sql|swf|url|xhtml|xml)\b|\b\w)/', array ('self', 'camelizeCallback' ), str_replace ( '_', ' ', $string ) );
		}
	 ...
	 *       	 The quantity (integer) or an array of objects to count
	 * @param $singular_form string
	 *       	 The string to be returned for when `$quantity = 1`
	 * @param $plural_form string
	 *       	 The string to be returned for when `$quantity != 1`, use `%d`
	 *        	to place the quantity in the string
	 * @param $use_words_for_single_digits boolean
		...
		if ($quantity == 1) {
			...
			if (strpos ( $output, '%d' ) !== FALSE) {
				...
				if ($use_words_for_single_digits && $quantity < 10) {
					...
					if (! $replacements) {
						$replacements = array (0 => self::compose ( 'zero' ), 1 => self::compose ( 'one' ), 2 => self::compose ( 'two' ), 3 => self::compose ( 'three' ), 4 => self::compose ( 'four' ), 5 => self::compose ( 'five' ), 6 => self::compose ( 'six' ), 7 => self::compose ( 'seven' ), 8 => self::compose ( 'eight' ), 9 => self::compose ( 'nine' ) );
					}
	...
	/**
	 * Returns the passed terms joined together using rule 2 from Strunk &
	 * White's 'The Elements of Style'
	 *
	 * @param $strings array
	 *       	 An array of strings to be joined together
	 * @param $type string
	 *       	 The type of join to perform, `'and'` or `'or'`
	...
	static public function joinArray($strings, $type) {
		...
		if (! in_array ( $type, $valid_types )) {
			throw new \Exception ( 'The type specified, %1$s, is invalid. Must be one of: %2$s.', $type, join ( ', ', $valid_types ) );
		}
	 ...
	 *
	 * @param $singular_noun string
	 *       	 The singular noun to pluralize
	 * @param $return_error boolean
	 *       	 If this is `TRUE` and the noun can't be pluralized, `FALSE`
	...
	static public function pluralize($singular_noun, $return_error = FALSE) {
		if (! strlen ( $singular_noun )) {
			throw new \Exception ( "An empty string was passed to %s", __CLASS__ . '::pluralize()' );
		}
		...
		if (! $plural_noun) {
			...
			throw new \Exception ( 'The noun specified could not be pluralized' );
		}
	 ...
	 *       	 The function to replace ::joinArray() with - should accept the
	 *        	same parameters and return the same type
	 * @return void
	 *
	 */
	...
	static public function reset() {
		...
		self::$singular_to_plural_rules = array ('([ml])ouse$' => '\1ice', '(media|info(rmation)?|news)$' => '\1', '(phot|log|vide)o$' => '\1os', '^(q)uiz$' => '\1uizzes', '(c)hild$' => '\1hildren', '(p)erson$' => '\1eople', '(m)an$' => '\1en', '([ieu]s|[ieuo]x)$' => '\1es', '([cs]h)$' => '\1es', '(ss)$' => '\1es', '([aeo]l)f$' => '\1ves', '([^d]ea)f$' => '\1ves', '(ar)f$' => '\1ves', '([nlw]i)fe$' => '\1ves', '([aeiou]y)$' => '\1s', '([^aeiou])y$' => '\1ies', '([^o])o$' => '\1oes', 's$' => 'ses', '(.)$' => '\1s' );
	}
	
	/**
	 * Returns the singular version of a plural noun
	 *
	 * @param $plural_noun string
	 *       	 The plural noun to singularize
	 * @param $return_error boolean
	 *       	 If this is `TRUE` and the noun can't be pluralized, `FALSE`
	 *        	will be returned instead
	...
	static public function singularize($plural_noun, $return_error = FALSE) {
		if (! strlen ( $plural_noun )) {
			throw new \Exception ( "An empty string was passed to %s", __CLASS__ . '::singularize()' );
		}
		...
		if (! $singular_noun) {
			...
			throw new \Exception ( 'The noun specified could not be singularized' );
		}
	 ...
	 * @return array The first element is the beginning part of the string, the
	 *         second element is the last word
	 *        
	 */
	...
	/**
	 * Uses the Porter Stemming algorithm to create the stem of a word, which is
	...
	static public function underscorize($string) {
		if (! strlen ( $string )) {
			throw new \Exception ( "An empty string was passed to %s", __CLASS__ . '::underscorize()' );
		}
